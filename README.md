...
**结论：改造前约 180–210 QPS；把"每请求一次落库"改成按周期批量之后，可信实测 ≈460–500 QPS（200 占比 100%），此时网关只用 7–8% CPU。**
...
- **改造前的实测上限 ≈190 QPS**（批量落库后为 ≈460–500 QPS，见下方云端复测）。
...
| **改造后：批量落库 + 关闭前落库** | **15 744**（⚠️ oha 口径，含成功计数的 503，仅作同机对比参考） | **3.40ms** | **151** | **314 742** |

（同机、同 mock、同客户端 `oha` 256 并发 × 20s。两档延迟**不可直接比**：改造前压根跑不到这个并发——它被写库串行化摁在 934 QPS，改造后才真正吃满 CPU；这里比的是 **QPS 16.9 倍** 与 **每请求 CPU**。库里的用量都是 SIGTERM 之后读出来的。）

**所以顺序是先修落库路径、再谈扩容**——不修的话加核/加实例也会在 190 QPS 附近走平，因为瓶颈是串行化的写路径而不是核数。（上表为本机盘的数字，fsync 仅 0.05ms。）

**云端复测（部署批量落库版之后，2 vCPU、每请求 1 事件）**：

> ⚠️ **本节曾出现过一组错误数字（1 400/1 559/1 593 QPS），已作废。** 原因：当时用 `oha`
> 测吞吐，而 `oha` 的 "Success rate" **只统计连接错误，503 也算成功**；那几档实测 503
> 占比高达 92–95%，所以"QPS"里绝大多数是被立即拒绝的请求。**口径已改为只数
> `/metrics` 里 `status="200"` 的增量**，并记录 200 占比。

| 并发 | **真实 QPS（只数 200）** | 200 占比 | gateway CPU | CPU/成功请求 |
|---|---|---|---|---|
| 32 | **464** | **100%** | 0.161 核（8%） | 0.346ms |
| 192 | **496** | **100%** | 0.149 核（7.5%） | 0.300ms |
| 768 | 51–73 | 3.6–5.4% | 0.26 核（13%） | —（无效档：agent 侧心跳超时导致大面积 503，见下） |

**结论**：可信的稳定吞吐是 **≈460–500 QPS（200 占比 100%）**，且此时网关只用 7–8% 的
CPU；768 并发那一档不可用，原因不在网关算力，而在 **agent 侧心跳超时→主动断开→重连握手
超时**的循环（见下）。

> ⚠️ **另一条同等重要的上限：云服务器的出口带宽。** 2026-09-18 实测这台 ECS 的**下行
...
>
> 也就是说上面那个 **≈475 QPS 恰好等于带宽上限**（475 × ~1 KB 响应 ≈ 3.8 Mbps），
> 而不是网关算力的上限——所以两个数字都要一起看：**网关自己的容量远高于此，链路先到顶**。
> 单流大响应的实测（256KB 走 616ms）与这个上限吻合。
>
> 推论（排障时最容易踩）：**带宽饱和会被 `head_timeout`(15s) 放大成"隧道坏掉"**——
> 响应头 15s 内到不了 → 摘除 agent → 连接被关 → agent 重连 → 期间注册表为空 → 全量 503。
> 实测把请求体从 1 KB 换到 8 KB（响应 ~11 KB），同一施加速率下失败率从 **0.00% 跳到 48%**，
> 日志里全是 `upstream head timeout; evicting agent`。**验收负载必须按字节预算设计**：
> 在 3.2 Mbps 下，`QPS × (请求+响应字节)` 不能超过 0.4 MB/s，否则测的是链路，不是网关。

**768 并发档的失败机制（已定位到代码）**：`crates/agent/src/lib.rs` 的 `heartbeat_loop`
给心跳回包只留 **5s**，与 5s 心跳间隔等长、零余量；满负载下开流要抢连接级流管理器而超时 →
心跳任务返回 Err → `run()` 的 `select!` 把"心跳结束"当致命 → **主动断开** → 重连又撞上
QUIC 默认 **10s 握手限时**（`s2n-quic-core/src/connection/limits.rs` 的
`MAX_HANDSHAKE_DURATION_DEFAULT`，可用 `Limits::with_max_handshake_duration` 调整）
→ 固定 1s 重试形成风暴。这期间网关注册表里没有健康 agent，`try_acquire` 全部过滤掉 →
每个请求 `503 "no edge available"`、`ttfb_ms=0`。

**因此新增了区分这两者的可观测性**（此前"注册表有 2 个 agent 但全部 stale"只能靠人肉推断）：
`hlmg_agents`（注册条目数，含失联未关连接的）与 **`hlmg_agents_healthy`**（心跳未过期、
真正可路由的）分开暴露，并新增 `hlmg_agent_rejections_total{reason=...}`
（`registry-empty` / `all-candidates-stale` / `no-agent-serves-model` / `all-candidates-at-capacity`）；
四种拒绝也各自打 `warn` 日志，带 `registered` / `healthy` / `oldest_last_seen_secs`。

**所以顺序是先修心跳、再谈扩容**：批量落库把写路径的瓶颈去掉了（每请求 CPU 从
0.55–0.9ms 降到 0.17–0.35ms），但高并发下的可用性现在卡在 agent 的心跳/重连策略上。
...
含义：**改造后单实例可信实测 ≈460–500 QPS（网关仅用 7–8% CPU），真实上限尚未探到**——因为当前高并发档受 agent 心跳/重连问题限制，探上限的前提是先修那里。真实 LLM 场景完全不受它约束：真实模型下 `QPS ≈ 并发 ÷ 单请求耗时`（几秒级），一两个数量级地低于这个数字；这些数字只在"把模型换快"或"拿网关转发非 LLM 小请求"时才有意义。
