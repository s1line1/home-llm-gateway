...
2. **单发超时**：`单请求字节 / 0.4 MB/s > head_timeout(15s)`，即**单请求 ≳ 6 MB** 时，
   ...
   ⚠️ **2026-09-22 修正**：可用速率会被并发**摊薄**，所以 N 个并发大请求的触发尺寸是 ≈ `6 MB / N`
   量级（实测 4 MB × 2 并发 = 共享 0.42 MB/s ⇒ 单个约 19 s ⇒ 全 504），不只是"单个 ≳ 6 MB"。
...
#### 请求体阶梯复测（2026-09-22，部署 `bb6351b` = PR #129）

再来一遍不同角度的确认：**这次让客户端也跑在 ECS 上**（打 `127.0.0.1:9090`），隧道仍跨公网回家里
那台 Mac（agent 与本地 mock-llm 都在 Mac 上），非流式 `chat/completions`——mock 会把 prompt 回吐，
所以**响应体 ≈ 请求体**，一次往返把两个方向都压到。

先量三条基线（都在本轮同一时间测的）：

| 测法 | 速率 |
|---|---|
| Mac → ECS（20 MB `scp` 上传） | **75 Mbps** |
| ECS → Mac（20 MB / 5 MB 两次） | **3.2 / 3.35 Mbps**（两次同值 ⇒ 是固定上限） |
| Mac ← 清华镜像站（20 MB，绕开该 ECS） | **16.6 MB/s ≈ 133 Mbps**（客户端侧不缺带宽） |

阶梯（每档 20–30 s，客户端是**闭环**：每个并发等自己的响应回来才发下一个）：

| 请求体 | 并发 | 请求数 | rps | 实测带宽 | p50 | 失败 |
|---|---|---|---|---|---|---|
| 8 KB | 64 | 1 072 | 51.3 | **0.44 MB/s** | 1.26 s | 0 |
| 8 KB | 128 | 1 102 | 50.9 | **0.44 MB/s** | 2.70 s | 0 |
| 64 KB | 32 | 160 | 6.4 | **0.42 MB/s** | 5.13 s | 0 |
| 256 KB | 16 | 48 | 1.6 | **0.42 MB/s** | 10.2 s | 0 |
| 1 MB | 4 | 12 | 0.4 | **0.42 MB/s** | 10.2 s | 0 |
| 4 MB | 2 | 4 | 0.1 | 0.56 MB/s（上行峰值） | 15.1 s | **4 × 504** |

**带宽恒定在 0.42 MB/s、与 payload 大小和并发都无关，而延迟随 payload 线性增长**
（8 KB→1.26 s、64 KB→5.1 s、256 KB→10.2 s）——这是"预算固定、排队变长"的形状，不是任何一端的算力上限。

**两个方向的对证**：把请求体设成 256 KB 但改打 `/v1/embeddings`（响应只有几百字节），得到的**上行**
（客户端发出的字节）依旧是 **0.42 MB/s**。所以卡住的是**请求下发这一跳（云出口）**——每个请求都必须
先经过它，闭环客户端因此把整条往返钉在 0.42 MB/s。

**4 MB 那 4 个 504**：`head_timeout` 到点，日志是
`WARN gateway::proxy::head: upstream head timeout while the agent is still answering; not evicting`
（`Options::DEFAULT_HEAD_TIMEOUT = 15s`）。两个 4 MB 并发**共享**同一条 0.42 MB/s ⇒ 单个要约 19 s
才送完 ⇒ 响应头来不及。这也修正了上面"单发超时"那条判据的算法：**阈值是"单请求字节 ÷ 可用速率"，
可用速率被并发摊薄**，所以 N 个并发大请求的触发尺寸是 ≈ `6 MB / N` 量级，不只是"单个 ≳ 6 MB"。

**与上面 2026-09-18 那组的差异（口径，不是矛盾）**：那天用 k6 的速率档施压（8 KB 那档折算 ≈1.4 MB/s，
是预算的 3.5 倍）⇒ 大面积 504；今天的客户端是闭环的，**它自己撑不到预算之上**（8 KB 档实测 0.44 MB/s，
正好贴着上限），所以 8 KB–1 MB 全部 0 失败。**要测"超预算会怎样"必须用开环/定速施压（k6 的 RATE、
或按字节预算算出来的并发数），闭环客户端测不出来。**

**同轮的正面证据**：全程 `hlmg_requests_aborted_total = 0`；4 MB 那批虽然 504，但日志是 `not evicting`、
`agent_rejections_total{reason="registry-empty"}` 为 0、agent 没有重连——即 2026-09-18 修好的
"慢 ≠ 死"判据在今天的部署上依旧成立：**链路饱和只产出若干 504，不再把整队拖成 503**。

**所以今天这份阶梯的结论与 2026-09-18 一致**：`QPS × (请求 + 响应字节) ≤ 0.4 MB/s` 是这台 ECS 上
