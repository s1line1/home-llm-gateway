...
**默认配置开着校验缓存**（`verified_cache_max: 1650`），所以稳态下 19MiB 那笔账几乎不发生：云端真实流量 21 516 个 200 请求 → `hits` 22 426 / `misses` **3**。缓存到底有没有在生效，看这两个计数器就知道：线上实测发 3 个带有效 key 的请求 → `hits=2 / misses=1`（首个 miss 跑 argon2，随后命中）。⚠️ `verified_cache_max: 0` 时两个数**恒为 0**（不走缓存路径就不加计数），看到 0 要先确认缓存是不是被关了，别当成"没有校验"。
...
**网关侧结论**（agent 侧的在下一节，链路带宽在《网关自身的吞吐上限》，三者别混着读）：
...
- **所以"加并发换不到吞吐"先查 agent**：判断顺序是 **agent 的事件速率 → 链路带宽 → 网关**。大 payload 时先撞的是**云出口带宽**（≈0.4 MB/s，见《网关自身的吞吐上限》）；网关自己的 CPU 在 500 QPS 时只用 7–8%。
...
CPU——**这 7–8% 就是关键**：上限不在网关算力，而在云服务器出口带宽（见下方带宽说明块；
496 × ~1 KB ≈ 4 Mbps ≈ 实测下行上限）。768 并发那一档**当时**不可用，原因也不在网关算力，而在
agent 侧心跳超时→主动断开→重连握手超时（**已修**，修复点见下）。
...
>
> 也就是说上面那个 **≈496 QPS 恰好等于带宽上限**（496 × ~1 KB 响应 ≈ 4 Mbps，
> 与实测下行 3.2–3.6 Mbps 同量级），而不是网关算力的上限——两个数字必须一起看：
> **网关自己的容量远高于此，是链路先到顶**。单流大响应实测（256 KB 走 616ms）与这个上限吻合。
>
...
**768 并发档曾不可用：心跳零余量 + 重连风暴（机制已定位，代码已修）**
