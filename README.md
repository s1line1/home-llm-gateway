...
### 网关自身的吞吐上限（实测：改造前 ≈190、改造后 ≥1 400 QPS / 2 vCPU）

**结论：改造前约 180–210 QPS；把"每请求一次落库"改成按周期批量之后，实测 ≥1 400 QPS 且网关只用掉 13% 的 CPU——上限已经不在网关，而在链路与客户端侧。**
...
- **（改造前的读数）gateway CPU 停在 2.0–2.3 核**，而云机 `nproc=2`、cgroup `cpu.max=max`（**没有配额限制**）、`nr_throttled=0`（没被节流）。当时看着像"算力打满"，2.1 核 ÷ 190 QPS ≈ **每请求 0.55–0.9ms CPU**；但下一节的剖析证明那不是算力用尽，而是每请求同步落库 + 全局锁争抢：**批量落库实施后，同样 2 vCPU 在 1 400 QPS 时只用 0.256 核、0.18ms/请求**。
- **这次不是别人拖后腿**（并发采三侧）：agent **1.05 核**、上游 mock 0.31 核（都可忽略）；链路 0.3MB/s 且 **UDP 丢包增量为 0**。
- **改造前的实测上限 ≈190 QPS**（对比 768/1 536 并发下的 1 559/1 593 QPS，见下方云端复测）。
...
**云端复测（部署批量落库版之后，2 vCPU、`oha`、每请求 1 事件）**：

| 并发 | QPS | 成功率 | avg | **gateway CPU** | **CPU/请求** |
|---|---|---|---|---|---|
| 192 | 493 | 99.99% | 386ms | 0.132 核（6.6%） | 0.268ms |
| **384** | **1 405–1 432** | **100.00%** | 205ms | 0.256 核（**13%**） | **0.182ms** |
| 768 | 1 559 | 98.96% | 189ms | 0.272 核（14%） | 0.175ms |
| 1 536 | 1 593 | 98.66% | 194ms | 0.272 核（14%） | 0.171ms |

四条读数：① **改造前 190 → 改造后 1 405–1 593 QPS，约 8 倍**（同口径：2 vCPU、每请求 1 事件）；② 并发从 384 加到 1 536（4 倍）QPS 只从 1 405 涨到 1 593，而**网关 CPU 还停在 14%**——瓶颈已转移到链路/压测端，不是网关算力；③ **每请求 CPU 从 0.55–0.9ms 降到 0.17–0.18ms**，正是去掉每请求同步落库的直接结果；④ 按 0.172ms/请求推算，**2 vCPU 用满约对应 ~11 600 QPS（推算，未实测）**——要实测必须把压测器放到云机本机，否则客户端先饱和。

⚠️ 同时暴露一个与本改动无关的既有问题：**768 并发以上 agent 会被网关摘除**（`tunnel_op_secs: 2s` 超时判定），而**摘除后 agent 不自动重连**，其后所有请求变 503。高并发段 98.7–99% 的成功率损失就来自这里，是下一步最该修的健壮性问题。
...
含义：**改造后单实例实测 ≥1 400 QPS（网关仅用 13% CPU），真实上限尚未探到**——要更高可以先加并发（链路允许的话），再加核或加实例（水平扩展见 `DESIGN.md` §11.4，未实现）。真实 LLM 场景完全不受它约束：真实模型下 `QPS ≈ 并发 ÷ 单请求耗时`（几秒级），一两个数量级地低于这个数字；这些数字只在"把模型换快"或"拿网关转发非 LLM 小请求"时才有意义。
