...
> 按三阶段实施。**状态：三阶段已实施完毕（2026-09）**——逐条勾选见下方四张表；唯一暂缓项是
> C3（转发零拷贝，理由见阶段 3 的说明），`A4`/`A5`/`C5` 属"明确不做"。
> （本行原先写"方案定稿，未实施"，与本文件自己的 ✅ 表和"阶段 1/2/3 已完成"直接矛盾，
> 2026-09-22 校正。）
...
| # | 优化项 | 现状 | 方案 | 优先级/工作量 |
|---|---|---|---|---|
| S1 ✅ | gateway crate 按职责拆模块 | ~~混合~~ → `proxy/mod.rs`（代理转发独立）、`storage/hash.rs`（哈希原语独立）、`config.rs`（配置解析独立）；http/mod.rs 只剩路由/中间件/fallback | 高 / 中 |
...
| C2 ✅ | keystore 写操作异步化 | ~~同步阻塞~~ → admin create_key/delete_key 走 `spawn_blocking`（argon2 哈希 + SQLite 写穿不卡 async worker） | 中 / 低 |
