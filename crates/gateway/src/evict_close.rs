...
//! 之间的那个上界。它的值由 `Options::evict_close_grace` 注入，**默认与 `head_timeout` 相等
//! （15s）**：宽限短于 `head_timeout` 时，宽限到期会掐断"仍在合法等响应头"的那一类请求
//! （配小了启动时 `Gateway::start` 会打一条 WARN）。它仍远小于 `request_timeout`(120s)，
//! 所以**长回答在被摘除的连接上依然会被切断**——这是本机制没有消除的取舍，不是搭配问题：
//! 保住多久＝宽限期有多长。这条契约由 `tests/e2e/evict_close.rs` 的两个 A/B 用例钉住
//! （宽限 1s → 在途生成被切断；宽限 30s → 它活着拿到 `[DONE]`）。
//!
